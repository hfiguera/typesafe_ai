defmodule TypeSafe.Bench.Server do
  ...
  def call(conn, opts) do
    ...
    response = Keyword.get(opts, :body, response)
  ...
  defp response([delay, "recorded", id, "v1", "systemone"], request, opts) do
    case Map.get(Keyword.get(opts, :recordings, %{}), id) do
      %{request: expected, response_body: body} ->
        if canonical_request(request) == canonical_request(expected) do
  ...
  def canonical_request(%{"questions" => questions} = request) do
    Map.put(
      request,
      "questions",
      Map.new(questions, fn {id, question} ->
        {id,
         if(question["criteria"] == nil, do: Map.delete(question, "criteria"), else: question)}
      end)
    )
  end
