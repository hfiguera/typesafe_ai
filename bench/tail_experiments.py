...
def run(plan_path, prefix):
    ...
    for rep, variant in schedule:
        ...
        command = variant.get('command_prefix', []) + command
        assert '--live' not in command
        env = os.environ.copy()
        env.pop('TYPESAFE_API_KEY', None)
        env['ERL_FLAGS'] = variant['flags']
        for key in ['TYPESAFE_BENCH_PATH', 'MIX_BUILD_PATH']:
            if key in variant.get('environment', {}):
                env[key] = variant['environment'][key]
        start = time.time()
        ...
        case['build_environment'] = {k: env[k] for k in ['TYPESAFE_BENCH_PATH', 'MIX_BUILD_PATH'] if k in env}
