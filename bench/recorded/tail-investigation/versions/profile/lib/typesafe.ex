defmodule TypeSafe do
  ...
  Designed for latency-sensitive applications. Evaluate shared state with Choice,
  Score, and Noul questions, and receive validated, typed answers.

  Start a `TypeSafe.Client` under your application's supervisor, then call
  `system_one/2`. Supervised connections, bounded concurrency and queues, request
  deadlines, configurable retries, and telemetry give you explicit control over
  how your application handles load. You receive `TypeSafe.Response` and typed
  answer structs. The client handles authentication and uses Mint directly.
  ...
  `client` is a PID or registered name on the calling node. Requests use local
  monotonic deadlines; remote-node client references return `:unavailable`.
