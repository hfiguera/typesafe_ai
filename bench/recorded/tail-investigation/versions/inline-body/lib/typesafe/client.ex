defmodule TypeSafe.Client do
  ...
  defp submit(state, id, request) do
    ...
    payload = if inline_body?(state.conn, body), do: body, else: :stream

    case Mint.HTTP.request(state.conn, "POST", path, headers, payload) do
      {:ok, conn, ref} ->
        request = %{request | ref: ref, upload: if(payload == :stream, do: body, else: nil)}
        ...
        state = %{
          ...
          | conn: conn,
            ...
            uploads:
              if(payload == :stream, do: MapSet.put(state.uploads, id), else: state.uploads),
  ...
  defp inline_body?(conn, body) when byte_size(body) <= @upload_chunk_bytes do
    case Mint.HTTP.protocol(conn) do
      :http1 ->
        true

      :http2 ->
        byte_size(body) <= Mint.HTTP2.get_window_size(conn, :connection) and
          byte_size(body) <= Mint.HTTP2.get_server_setting(conn, :initial_window_size)
    end
  end

  defp inline_body?(_conn, _body), do: false
