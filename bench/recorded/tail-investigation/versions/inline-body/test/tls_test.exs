defmodule TypeSafe.TLSTest do
  ...
  test "small HTTP/2 requests finish in their data frame without an empty end frame" do
    owner = self()

    server =
      server(
        {:raw,
         fn transport, socket -> H2Server.serve(transport, socket, owner, refill: false) end},
        alpn_preferred_protocols: ["h2"]
      )

    client = client(server)
    request = Task.async(fn -> evaluate(client) end)
    assert_receive {:h2_request, worker, stream, headers, body}, 2_000
    assert JSON.decode!(body)["state"] == "hello"
    assert {"content-length", Integer.to_string(byte_size(body))} in headers
    assert_receive {:h2_data, ^worker, ^stream, bytes}
    assert bytes == byte_size(body)
    refute_receive {:h2_data, ^worker, ^stream, _}, 20
    send(worker, {:respond, stream, TestServer.success()})
    assert {:ok, _} = Task.await(request)
  end

  test "a small body exceeding the peer stream window still waits for credit" do
    owner = self()
    ...
    server =
      server(
        {:raw,
         fn transport, socket ->
           H2Server.serve(transport, socket, owner, window: 1024, refill: false)
         end},
        alpn_preferred_protocols: ["h2"]
      )
    ...
    assert_receive {:h2_request, worker, stream, _, _}, 2_000
    send(worker, {:respond, stream, TestServer.success()})
    assert {:ok, _} = Task.await(warm)

    state = String.duplicate("x", 2000)
    request = Task.async(fn -> evaluate(client, state: state) end)
    assert_receive {:h2_data, ^worker, blocked, 1024}, 1000
    refute_receive {:h2_request, ^worker, ^blocked, _, _}, 20
    send(worker, {:window_update, blocked, 4096})
    assert_receive {:h2_request, ^worker, ^blocked, _, body}, 1000
    assert JSON.decode!(body)["state"] == state
    send(worker, {:respond, blocked, TestServer.success()})
    assert {:ok, _} = Task.await(request)
  end

  test "a small body exceeding connection credit falls back to streaming" do
    owner = self()
    ...
    server =
      server(
        {:raw,
         fn transport, socket ->
           H2Server.serve(transport, socket, owner, window: 65_535, refill: false)
         end},
        alpn_preferred_protocols: ["h2"]
      )
    ...
    worker =
      Enum.reduce(1..4, nil, fn _, _ ->
        request = Task.async(fn -> evaluate(client, state: String.duplicate("x", 16_000)) end)
        assert_receive {:h2_request, worker, stream, _, _}, 2000
        send(worker, {:respond, stream, TestServer.success()})
        assert {:ok, _} = Task.await(request)
        worker
      end)
    ...
    remaining =
      client |> :sys.get_state() |> Map.fetch!(:conn) |> Mint.HTTP2.get_window_size(:connection)

    assert remaining > 0 and remaining < 2000
    state = String.duplicate("x", 2000)
    request = Task.async(fn -> evaluate(client, state: state) end)
    assert_receive {:h2_data, ^worker, blocked, ^remaining}, 1000
    refute_receive {:h2_request, ^worker, ^blocked, _, _}, 20
    send(worker, {:window_update, blocked, 4096})
    assert_receive {:h2_request, ^worker, ^blocked, _, body}, 1000
    assert JSON.decode!(body)["state"] == state
