...
The rubric is 8 `Choice`s, and each run is one call that answers all 8. We do 15
...
* TypeSafe: one `system_one` call over the 8 `Choice`s, with a fresh `uid` field (a
...
`Choice`s, returning one distribution per question.
